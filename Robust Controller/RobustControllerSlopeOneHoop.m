...
function []= RobustControllerSlopeOneHoop(theta10,theta20,dtheta10,dtheta20,...
                              theta1f,theta2f1,dtheta1f,dtheta2f,tf)
clc; close all;
...
I3 = .0006; 
g = 9.81;
l = 0.12; %Pendulum length
eta = 0.03; 
gamma = 5*pi/180;
theta2f = asin((m1+m2+m3)*R*sin(gamma)/m3*l);
...
%axis([0 5 -5 5]);
    ...
    function [dx ] = planarArmODEUncertain(t,x)
        ...
        % C
        m11 = (m1+m2+m3)*R^2 + I1;
        m12 = m3*R*l*cos(x(2)-gamma);
        m21 = m3*R*l*cos(x(2)-gamma);
        m22 = m3*l^2+I2+I3;
        Mmat = [ m11, m12; m21, m22];
        Cmat = [ eta, eta - m3*R*l*sin(x(2)-gamma)*x(4);  
                 eta, eta];
        Gmat = [ (m1+m2+m3)*R*g*sin(gamma); m3*g*l*sin(x(2))];
        ...
        Y2= [ a(1), g*sin(gamma), a(2)*cos(gamma-x(2)) + x(4)*v(2)*sin(gamma-x(2)), 0, v(1) + v(2), 0;
                 0,    0,          a(1)*cos(gamma-x(2)), a(2), v(1) + v(2), g*sin(x(2))];
        epsilon=8;
        rho =  8;%13.46; % see eq(27) in the paper.
...
while i <=length(T) %Display every frame
    showHoop(animeFigure,[0,0],R,X(i,1), X(i,2),l,gamma); % showSate plots the frame
