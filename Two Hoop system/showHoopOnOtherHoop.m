...
clf;
hold on
gamma = b_thetaHoop +t_radHoop/b_Radius * t_thetaHoop;
...
%motion:
theta = linspace(0,2*pi,5);
CircX=t_radHoop*cos(theta-t_thetaHoop-gamma)+Xo;
CircY=t_radHoop*sin(theta-t_thetaHoop-gamma)+Yo;
plot(CircX,CircY,'.','color','r'); 
...
%axis([(floor(center(1)/(2*pi*(t_radHoop+b_Radius))))*2*pi*(+b_Radius)-(t_radHoop+b_Radius);(floor(center(1)/(2*pi*(t_radHoop+b_Radius)))+1)*2*pi*(+b_Radius)+(t_radHoop+b_Radius); (floor(center(2)/(2*pi*(t_radHoop+b_Radius))))*2*pi*(+b_Radius)-(t_radHoop+b_Radius);(floor(center(2)/(2*pi*(t_radHoop+b_Radius)))+1)*2*pi*(+b_Radius)+(t_radHoop+b_Radius);]);
window = 1.5*[-(2*t_radHoop+b_Radius);(2*t_radHoop+b_Radius);-(2*t_radHoop);(2*t_radHoop+2*b_Radius)];

r = t_radHoop+b_Radius;
offset = [floor(centerL(1)/r)*r-(2*t_radHoop+b_Radius);floor(centerL(1)/r)*r-(2*t_radHoop);0;0];
axes = window+offset;
axis (axes)
%show ground line 
plot([axes(1);axes(2)],[0;0]);
hold off
